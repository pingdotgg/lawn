...
- The worker claims at most 10 resources per action, acknowledges each separately,
...
- Mux passthrough includes the video and upload key. Callbacks accept only the
...
execution still need observation after deployment. The isolated validation
checkout has no Convex deployment configured, so deployment-backed code generation
was not run; the new API module declarations are included and typechecked.
